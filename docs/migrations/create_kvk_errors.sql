-- Create kvk_errors table for KvK error reports
-- Run this in Supabase Dashboard → SQL Editor
-- This is for user-reported errors about KvK data (wrong opponent, wrong result, etc.)
-- Different from kvk_corrections which is for approved fixes to KvK records

-- 1. Create the kvk_errors table
CREATE TABLE IF NOT EXISTS public.kvk_errors (
  ...
  kvk_number INTEGER, -- NULL for missing_kvk error type
  ...
  current_data JSONB, -- Snapshot of current data at time of report
...
CREATE INDEX IF NOT EXISTS idx_kvk_errors_kvk_number ON public.kvk_errors(kvk_number);
...
-- Notify admins on new error report
...
-- Notify user when their error report is reviewed
