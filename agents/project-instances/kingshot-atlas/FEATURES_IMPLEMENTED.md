...
| Referral Count Integrity System | ✅ Live | Platform | Prevents referral count discrepancies via 4 mechanisms: (1) `sync_referral_count_on_status_change` trigger — AFTER UPDATE/DELETE on `referrals`, recounts verified referrals and syncs `profiles.referral_count`+`referral_tier`. (2) `referral_audit_log` table — tracks all status changes (INSERT/UPDATE/DELETE) with context (ip_abuse_auto_invalid, status_change, row_deleted, new_referral). Admin-only RLS. (3) Invalidation notifications — `referral_invalidated` notification type sent to referrer when a referral is flagged. Amber ⚠️ icon, links to /profile. (4) `reconcile_referral_counts()` function — weekly pg_cron job (Sundays 04:00 UTC) fixes any remaining drift. Initial run fixed 1 mismatched profile. (2026-07-17) |
