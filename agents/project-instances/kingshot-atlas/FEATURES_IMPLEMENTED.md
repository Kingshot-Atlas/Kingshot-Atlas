...
| Atlas Bot Dashboard | ✅ Built | `/atlas-bot/dashboard` — guild management page for Discord server admins. Supabase tables: `bot_guild_settings`, `bot_guild_admins`, `bot_alliance_events` (RLS, auto-triggers). 4 Alliance Event cards (Bear Hunt daily, Viking Vengeance biweekly Tue/Thu, Swordland Showdown biweekly Sun, Tri-Alliance Clash monthly Sat). UTC time slot editor, reminder advance config (5/10/15/30/60m), per-event channel/role overrides. Gift Code Alerts toggle. Auth: sign-in + Discord link required. Status strip dashboard. Link from `/atlas-bot` landing page. (2026-02-21) |
