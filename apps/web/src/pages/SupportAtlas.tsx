...
import { SUPPORTER_PERKS } from '../utils/constants';
...
const SupportAtlas: React.FC = () => {
  ...
  const supporterPerks = SUPPORTER_PERKS.map(p => ({
    icon: p.icon,
    text: t(`support.perk${p.key}Title`),
    desc: t(`support.perk${p.key}Desc`),
  }));
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: isMobile ? '1.5rem 1rem' : '2rem' }}>
        ...
        <section style={{ 
          ...
          <p style={{ color: '#9ca3af', fontSize: isMobile ? '0.85rem' : '0.9rem', lineHeight: 1.6, marginBottom: '1.5rem', maxWidth: '500px', margin: '0 auto 1.5rem' }}>
            {t('support.getPerks', 'Badge, Discord role, Alliance Center, Bot Dashboard, early access — and the satisfaction of funding the tools you actually use.')}
          </p>
