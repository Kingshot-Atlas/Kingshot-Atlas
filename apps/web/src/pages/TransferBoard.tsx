...
const TransferBoard: React.FC = () => {
  ...
  const [dismissedDiscordCallout, setDismissedDiscordCallout] = useState(() => localStorage.getItem('atlas_dismiss_discord_transfer') === 'true');
      ...
      {!loading && atlasPlayerCount > 0 && (
        <div style={{
          ...
          <div style={{
            ...
            background: 'linear-gradient(135deg, #22d3ee08 0%, #a855f708 100%)',
            border: '1px solid #22d3ee20',
            borderRadius: '12px',
          }}>
            <span style={{ fontSize: '1rem' }}>🌍</span>
            <span style={{ color: colors.textSecondary, fontSize: '0.8rem', fontWeight: '500' }}>
              {t('transferHub.playersInAtlas', 'Kingshot Players in Atlas:')}
            </span>
            <span style={{
              color: '#22d3ee',
              fontSize: '1.1rem',
              ...
              {atlasPlayerCount.toLocaleString()}
            </span>
          </div>
        </div>
      )}

      {/* Discord Transfer Group Callout */}
      {!dismissedDiscordCallout && (
        <div id="discord-transfer-callout" style={{
          padding: isMobile ? '0.75rem 1rem' : '0.75rem 1.25rem',
          marginBottom: '1rem',
          backgroundColor: '#5865F208',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ color: '#e5e7eb', fontSize: isMobile ? '0.8rem' : '0.85rem', fontWeight: '600', margin: 0, lineHeight: 1.4 }}>
                {t('transferHub.discordCalloutTitle', 'Transfer Group channels are live in Discord')}
              </p>
              <p style={{ color: '#9ca3af', fontSize: isMobile ? '0.7rem' : '0.75rem', margin: '0.15rem 0 0 0', lineHeight: 1.4 }}>
                {t('transferHub.discordCalloutDesc', 'Coordinate directly with players in your kingdom range. Each group has its own channel.')}
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
            ...
            >
              {t('transferHub.joinTransferGroup', 'Join Discord')}
            </a>
            <button
              onClick={() => {
                localStorage.setItem('atlas_dismiss_discord_transfer', 'true');
                setDismissedDiscordCallout(true);
              }}
