...
interface EditRow {
  ...
  isNew: boolean; // true = roster member not yet in list
}
...
interface BearBulkEditProps {
  existingPlayers: BearPlayerEntry[];
  unscoredNames: string[];
  onSave: (updatedPlayers: BearPlayerEntry[]) => void;
  onClose: () => void;
  isMobile: boolean;
}
...
const BearBulkEdit: React.FC<BearBulkEditProps> = ({ existingPlayers, unscoredNames, onSave, onClose, isMobile }) => {
  const { t } = useTranslation();

  // Build initial rows from existing players + unscored roster members
  const [rows, setRows] = useState<EditRow[]>(() => {
    const existing: EditRow[] = existingPlayers.map(p => ({
      id: p.id,
    ...
    const unscored: EditRow[] = unscoredNames.map(name => ({
      id: genId(),
      playerName: name,
      infantryHero: '',
      infantryEGLevel: -1,
  ...
  const completeRows = useMemo(() =>
    rows.filter(r =>
      ...
      r.infantryAttack && r.infantryLethality &&
      r.cavalryAttack && r.cavalryLethality &&
      r.archerAttack && r.archerLethality
    ),
    [rows]
  );

  const handleSave = useCallback(() => {
    // Keep all rows with a player name
    ...
    // Determine which rows have any data entered (vs just a name placeholder)
    ...
    // Validate rows that have partial data — require completion
    for (const r of namedRows) {
      if (!hasAnyData(r)) continue; // empty placeholder — saved as incomplete
    ...
    // Build entries — incomplete rows get score 0, complete rows get calculated score
    const entries: BearPlayerEntry[] = namedRows.map(r => {
      ...
      const isComplete = hasAnyData(r);
      const bearScore = isComplete ? calculateBearScore(
      ...
      return {
        ...
        infantryEGLevel: r.infantryEGLevel >= 0 ? r.infantryEGLevel : 0,
        infantryAttack: infAtk,
        infantryLethality: infLeth,
        cavalryHero: r.cavalryHero,
        cavalryEGLevel: r.cavalryEGLevel >= 0 ? r.cavalryEGLevel : 0,
        cavalryAttack: cavAtk,
        cavalryLethality: cavLeth,
        archerHero: r.archerHero,
        archerEGLevel: r.archerEGLevel >= 0 ? r.archerEGLevel : 0,
    ...
    // Recalculate tiers only for complete players
    const complete = entries.filter(isPlayerComplete);
    const allScores = complete.map(p => p.bearScore);
    const withTiers = entries.map(p => isPlayerComplete(p) ? { ...p, tier: assignBearTier(p.bearScore, allScores) } : p);
  ...
  const numInput: React.CSSProperties = { ...cellInput, minWidth: '68px' };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 700, color: '#fff' }}>
            ✏️ {t('bearRally.bulkEditTitle', 'Edit All Players')}
          </h2>
        ...
        <p style={{ color: '#6b7280', fontSize: '0.75rem', marginBottom: '0.75rem', lineHeight: 1.5 }}>
          {t('bearRally.bulkEditDesc', 'Edit data for all players. Roster members without data will be skipped on save.')}
        </p>
        ...
        {rows.map((row, idx) => (
          <div key={row.id} style={{
            ...
            borderRadius: '12px',
            border: `1px solid ${row.isNew ? `${ACCENT}20` : '#2a2a2a'}`,
            padding: '0.75rem',
            marginBottom: '0.5rem',
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 700, color: row.isNew ? '#6b7280' : '#fff' }}>
                ...
                {row.isNew && <span style={{ fontSize: '0.6rem', color: '#4b5563', marginLeft: '0.4rem' }}>{t('bearRally.unscoredLabel', '(no data)')}</span>}
              </span>
            </div>
            {/* Heroes & Gear section */}
        ...
        <button onClick={handleSave} style={{
          ...
          {t('bearRally.bulkEditSave', 'Save Changes ({{count}} players)', { count: completeRows.length })}
        </button>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
        ...
        {unscoredNames.length > 0 && (
          <span style={{ fontSize: '0.7rem', color: '#6b7280', flexShrink: 0 }}>
            ✦ = {t('bearRally.unscoredLegend', 'roster member without data')}
          </span>
        )}
      </div>
    </div>
