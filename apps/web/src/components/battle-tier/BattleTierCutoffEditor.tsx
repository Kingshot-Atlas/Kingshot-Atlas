...
interface BattleTierCutoffEditorProps {
  mode: 'offense' | 'defense';
  rankedPlayers: (BattlePlayerEntry & { rank: number })[];
  tierOverrides: BattleTierOverrides | null;
  autoBoundaries: { SS: number; S: number; A: number; B: number; C: number } | null;
  onSetOverrides: (overrides: BattleTierOverrides | null) => void;
  canEdit: boolean;
  isMobile: boolean;
}

const BattleTierCutoffEditor: React.FC<BattleTierCutoffEditorProps> = ({
  mode,
  rankedPlayers,
  tierOverrides,
  autoBoundaries,
  ...
  const tierCounts = useMemo(() => {
    const counts: Record<BattleTier, number> = { SS: 0, S: 0, A: 0, B: 0, C: 0, D: 0 };
    for (const p of rankedPlayers) {
      if (isPlayerComplete(p)) {
        const tier = mode === 'offense' ? p.offenseTier : p.defenseTier;
        counts[tier]++;
      }
    }
    return counts;
  ...
  const tierFirstPlayer = useMemo(() => {
    const first: Partial<Record<BattleTier, string>> = {};
    for (const p of rankedPlayers) {
      const tier = mode === 'offense' ? p.offenseTier : p.defenseTier;
      if (!first[tier] && isPlayerComplete(p)) {
        first[tier] = p.playerName;
      }
    }
    return first;
  }, [rankedPlayers, mode]);
  ...
  const handleApply = useCallback(() => {
    ...
    const newOverrides: BattleTierOverrides = {
    ...
    <div style={{
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {t('battleTier.tierCutoffs', 'Tier Cutoffs')}
          {tierOverrides && (
            <span style={{
              fontSize: '0.6rem',
              ...
              {t('battleTier.customLabel', 'CUSTOM')}
            </span>
          )}
        </span>
      ...
      {!isOpen && activeBoundaries && (
        <div style={{
          ...
          <span style={{
            ...
            <span style={{ color: BATTLE_TIER_COLORS.D, fontWeight: 700, fontFamily: FONT_DISPLAY }}>D</span>
      ...
      {isOpen && (
        <div style={{ padding: '0 0.85rem 0.85rem' }}>
          <p style={{ fontSize: '0.68rem', color: '#666', margin: '0 0 0.75rem', lineHeight: 1.4 }}>
            {t('battleTier.tierCutoffsDesc', 'Set the minimum score for each tier. Players below the C threshold are placed in D tier.')}
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              return (
                <div key={tier} style={{
                  ...
                  <span style={{
                    ...
                    {tierCounts[tier]} {tierCounts[tier] === 1 ? t('battleTier.player', 'player') : t('battleTier.players', 'players')}
                  </span>
            ...
            <div style={{
              ...
              gap: '0.5rem',
              padding: '0.4rem 0.5rem',
              backgroundColor: `${BATTLE_TIER_COLORS.D}08`,
              borderRadius: '6px',
              border: `1px solid ${BATTLE_TIER_COLORS.D}15`,
              opacity: 0.7,
            }}>
              <span style={{
                display: 'inline-flex',
                ...
                minWidth: '32px',
                height: '24px',
                backgroundColor: `${BATTLE_TIER_COLORS.D}18`,
                border: `1px solid ${BATTLE_TIER_COLORS.D}40`,
                borderRadius: '5px',
                color: BATTLE_TIER_COLORS.D,
                fontSize: '0.65rem',
                fontWeight: 800,
                fontFamily: FONT_DISPLAY,
              }}>
              ...
              <span style={{ fontSize: '0.65rem', color: '#666' }}>
                {t('battleTier.belowC', 'Below C threshold')}
              </span>
              <span style={{ fontSize: '0.65rem', color: '#888', textAlign: 'center', whiteSpace: 'nowrap' }}>
                {tierCounts.D} {tierCounts.D === 1 ? t('battleTier.player', 'player') : t('battleTier.players', 'players')}
              </span>
              {!isMobile && <span />}
            </div>
          </div>
          ...
          <div style={{
            ...
            {tierOverrides && (
              ...
              >
                {t('battleTier.resetToAuto', 'Reset to Auto')}
              </button>
            )}
            <button
              onClick={handleApply}
            ...
            >
              {t('battleTier.applyCutoffs', 'Apply Cutoffs')}
            </button>
          </div>
        </div>
      )}
...
export default BattleTierCutoffEditor;
