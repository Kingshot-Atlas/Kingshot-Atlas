...
@router.get("/stats/plausible/timeseries")
async def get_plausible_timeseries(
    period: str = "30d",
    x_admin_key: Optional[str] = Header(None),
    authorization: Optional[str] = Header(None)
):
    """Get daily visitors + pageviews timeseries from Plausible for growth charts."""
    require_admin(x_admin_key, authorization)
    if not PLAUSIBLE_API_KEY:
    ...
    try:
        import urllib.request
        import json as json_lib
        url = f"https://plausible.io/api/v1/stats/timeseries?site_id={PLAUSIBLE_SITE_ID}&period={period}&metrics=visitors,pageviews"
        req = urllib.request.Request(url, headers={"Authorization": f"Bearer {PLAUSIBLE_API_KEY}"})
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json_lib.loads(resp.read().decode())
        return {"results": data.get("results", []), "period": period}
    except Exception as e:
        logger.warning(f"Plausible timeseries error: {e}")
...
async def get_user_growth(
    ...
    """Get daily user signup counts and cumulative user breakdown over last 30 days from Supabase."""
    require_admin(x_admin_key, authorization)
    client = get_supabase_admin()
    if not client:
        return {"error": "Database unavailable", "signups": [], "breakdown_history": []}
    try:
        from datetime import datetime, timedelta, timezone
        now = datetime.now(timezone.utc)
        thirty_days_ago = (now - timedelta(days=30)).strftime("%Y-%m-%dT00:00:00+00:00")

        # Get all users with created_at in last 30 days for signup growth
        users_resp = client.table("profiles").select(
            "id,created_at"
        ).gte("created_at", thirty_days_ago).order("created_at").execute()
        ...
        for u in users:
            ...
            if day in signup_map:
                signup_map[day] += 1
        signups = [{"date": k, "count": v} for k, v in signup_map.items()]

        # Get total user count for cumulative
        total_resp = client.table("profiles").select("id", count="exact").execute()
        total_users = total_resp.count or 0

        # Build cumulative totals (work backwards from current total)
        cumulative = []
        running = total_users
        for entry in reversed(signups):
            cumulative.insert(0, {"date": entry["date"], "total": running})
            running -= entry["count"]

        # Get current user breakdown by subscription tier
        user_data_resp = client.table("user_data").select(
            "subscription_tier"
        ).execute()
        user_rows = user_data_resp.data or []
        tier_counts = {"free": 0, "supporter": 0, "recruiter": 0}
        ...
        # Kingshot linked count
        linked_resp = client.table("user_data").select("id", count="exact").not_.is_("kingshot_id", "null").execute()
        kingshot_linked = linked_resp.count or 0
        ...
        return {
            ...
            "breakdown": {
                "free": tier_counts["free"],
                "supporter": tier_counts["supporter"],
                "recruiter": tier_counts["recruiter"],
                "kingshot_linked": kingshot_linked,
            }
        }
    except Exception as e:
        logger.warning(f"User growth error: {e}")
        return {"error": str(e), "signups": [], "cumulative": [], "breakdown": {}}
